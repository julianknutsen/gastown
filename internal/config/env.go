...
type AgentEnvConfig struct {
	...
	// ResolvedAgent is the agent name resolved from rig/town config via
	// ResolveRoleAgentConfig. Used as a fallback for GT_AGENT when Agent
	// (explicit --agent override) is empty. This ensures all roles — not
	// just polecats — have GT_AGENT in their tmux session table for
	// accurate process detection by IsAgentAlive and witness patrol.
