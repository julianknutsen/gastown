...
// TestMultiTownSessionNaming tests session naming with town ID suffixes.
// This validates the optimistic-with-retry lookup pattern for backwards compatibility.
func TestMultiTownSessionNaming(t *testing.T) {
	tmx := NewTmux()

	t.Run("sessions with town ID suffix", func(t *testing.T) {
		// Simulate new-style session name with 6-char hex suffix
		name := testPrefix + "mayor-abc123"
		t.Cleanup(func() {
			_ = tmx.Stop(session.SessionID(name))
		})
		...
		if !exists {
			t.Error("session with town ID suffix should exist")
		}

		// Should NOT find without suffix (exact match only)
		legacyID := session.SessionID(testPrefix + "mayor")
		exists, _ = tmx.Exists(legacyID)
		if exists {
			t.Error("should not find suffixed session by legacy name")
		}
	})

	t.Run("legacy sessions without suffix", func(t *testing.T) {
		// Simulate legacy session name without suffix
		name := testPrefix + "deacon"
		t.Cleanup(func() {
			_ = tmx.Stop(session.SessionID(name))
		})
		...
		exists, _ := tmx.Exists(id)
		if !exists {
			t.Error("legacy session should exist")
		}

		// Should NOT find with suffix appended
		newID := session.SessionID(name + "-abc123")
		exists, _ = tmx.Exists(newID)
		if exists {
			t.Error("should not find legacy session by suffixed name")
		}
	})

	t.Run("optimistic lookup pattern", func(t *testing.T) {
		// This tests the pattern callers should use:
		// 1. Try new format first (with suffix)
		// 2. Fall back to legacy format if not found

		legacyName := testPrefix + "witness-legacy"
		t.Cleanup(func() {
			_ = tmx.Stop(session.SessionID(legacyName))
		})

		// Create a legacy-style session
		_, err := tmx.Start(legacyName, "", "sleep 60")
		if err != nil {
			t.Fatalf("Start failed: %v", err)
		}

		// Simulate optimistic lookup: try new format first
		newFormatID := session.SessionID(legacyName + "-def456")
		exists, _ := tmx.Exists(newFormatID)

		var foundID session.SessionID
		if exists {
			foundID = newFormatID
		} else {
			// Fall back to legacy format
			legacyID := session.SessionID(legacyName)
			exists, _ = tmx.Exists(legacyID)
			if exists {
				foundID = legacyID
			}
		...
		if foundID == "" {
			t.Error("optimistic lookup should find legacy session on retry")
		}
		if string(foundID) != legacyName {
			t.Errorf("expected to find %q, got %q", legacyName, foundID)
		}
	})

	t.Run("List returns both formats", func(t *testing.T) {
		legacy := testPrefix + "list-legacy"
		suffixed := testPrefix + "list-new-aabbcc"

		t.Cleanup(func() {
			_ = tmx.Stop(session.SessionID(legacy))
			_ = tmx.Stop(session.SessionID(suffixed))
		})

		_, _ = tmx.Start(legacy, "", "sleep 60")
		_, _ = tmx.Start(suffixed, "", "sleep 60")
		...
		foundLegacy, foundSuffixed := false, false
		for _, id := range sessions {
			if string(id) == legacy {
				foundLegacy = true
			}
			if string(id) == suffixed {
				foundSuffixed = true
			}
		}

		if !foundLegacy {
			t.Error("List should include legacy session")
		}
		if !foundSuffixed {
			t.Error("List should include suffixed session")
		}
	})

	t.Run("rig-level sessions with suffix", func(t *testing.T) {
		// Test gt-{rig}-{role}-{townID} format
		name := testPrefix + "myrig-witness-112233"
		t.Cleanup(func() {
			_ = tmx.Stop(session.SessionID(name))
		})
		...
		exists, _ := tmx.Exists(id)
		if !exists {
			t.Error("rig-level session with suffix should exist")
		}
	})

	t.Run("crew sessions with suffix", func(t *testing.T) {
		// Test gt-{rig}-crew-{name}-{townID} format
		name := testPrefix + "myrig-crew-max-445566"
		t.Cleanup(func() {
			_ = tmx.Stop(session.SessionID(name))
		})
		...
		exists, _ := tmx.Exists(id)
		if !exists {
			t.Error("crew session with suffix should exist")
		}
	})
}
