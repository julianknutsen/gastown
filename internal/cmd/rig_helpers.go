...
func getRig(rigName string) (string, *rig.Rig, error) {
	...
	// Update Polecats from polecat backend (supports remote rigs)
	polecatBackend := polecat.BackendFor(r, git.NewGit(r.Path))
	if names, listErr := polecatBackend.ListPolecatNames(); listErr == nil {
		r.Polecats = names
	}
