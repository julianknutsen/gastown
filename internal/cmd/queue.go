...
Queue work:
  ...
  gt convoy queue hq-cv-abc               # Queue convoy's issues
...
func runQueueStatus(cmd *cobra.Command, args []string) error {
	...
	activePolecats := countActivePolecats(townRoot)
...
func listQueuedBeads(townRoot string) ([]queuedBeadInfo, error) {
	...
	for _, r := range raw {
		result = append(result, queuedBeadInfo{
			...
			// Blocked status would require cross-rig lookup; skip for now
		})
	}
	return result, nil
}
...
// detectActor is defined in sling_helpers.go
