...
// countRunningPolecats counts the number of active polecats across all rigs.
// Uses the polecat manager's ActivePolecatCount() which reconciles with existing directories.
func countRunningPolecats(townRoot string) int {
	...
	for rigName := range rigsConfig.Rigs {
		...
		mgr := polecat.NewManager(r, polecatGit, nil) // nil tmux: just counting
		mgr.ReconcilePool()
		total += mgr.ActivePolecatCount()
	}
	return total
}
